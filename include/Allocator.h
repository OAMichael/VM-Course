...
public:
    ...
    inline T* getMemoryPtr() {
        uint8_t* memory = getVirtualMachineMemory();
        if constexpr (memT == MemoryType::Program) {
            return (T*)(memory + VM_PROGRAM_MEMORY_ADDRESS);
        }

        if constexpr (memT == MemoryType::ConstantPool) {
            return (T*)(memory + VM_CONSTANT_POOL_MEMORY_ADDRESS);
        }

        if constexpr (memT == MemoryType::StringPool) {
            return (T*)(memory + VM_STRING_POOL_MEMORY_ADDRESS);
        }

        if constexpr (memT == MemoryType::ClassDescs) {
            return (T*)(memory + VM_CLASS_DESCS_MEMORY_ADDRESS);
        }

        if constexpr (memT == MemoryType::Arena) {
            return (T*)(memory + VM_ARENA_MEMORY_ADDRESS);
        }
